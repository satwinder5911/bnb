       ...
       <?php
          ...
          if(isset($_POST['update']))
        {
            $room=$_POST['room'];
              ...
              $sql = "UPDATE bookings SET room='$room',checkindate='$checkinDate',checkoutdate='$checkoutDate',contactno=$contactNumber,extra='$bookingExtras',review='$review' WHERE booking_id=$booking_id";
            ...
            $sql = "SELECT * FROM `rooms`";
             $result = $conn->query($sql);
              ...
              if($result->num_rows > 0) 
              {
                ...
                {
                    ?>
                  <option value="<?= $row["room_name"]?>"><?= $row["room_name"]?> <?= $row["roomtype"]?> <?= $row["beds"]?></option>
                  <?php
              } 
            }
          ?>

...
if(isset($_POST['add']))
{
  ...
  $review=$_POST['review'];
  $sql = "INSERT INTO bookings (customer,room,checkindate,checkoutdate,contactno,extra,review) VALUES ('$customer', '$room', '$checkinDate','$checkoutDate',$contactNumber,'$bookingExtras','$review')";
        ...
        $sql = "SELECT * FROM `rooms`";
         $result = $conn->query($sql);
          ...
          if($result->num_rows > 0) 
          {
            ...
            {
              echo '<option value="'.$row["room_name"].'">'.$row["room_name"].' '.$row["roomtype"].' '.$row["beds"].'</option>';
          } 
        }
      ?>
 </select>
